...
Пересекатор
--------------------
Инструмент по заданной геометрии пересекает все слои веб-карты nextgis.com и формирует отчет, в котором перечисляются слои, с которыми состоялось пересечение. Если в отдельном слое объект пересекается несколько раз или пересекаются разные объекты, в отчете эти случаи отображаются как самостоятельные события.
 ...
 *  url - http://amery.nextgis.com
 *  webmap_id - 27 (так как адрес веб-карты http://amery.nextgis.com/resource/27/display)
