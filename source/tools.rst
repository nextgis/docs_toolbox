...
------------------------------------
Инструмент изменяет значение целевого атрибута для указанного объекта слоя в заданном ресурсе Веб ГИС по идентификатору.
Например, в группе ресурсов находятся три взаимосвязанных слоя. Нужно изменить значения поля 'fid2' на 1112 для всех объектов с идентификатором 1216 (поле 'fid').

На входе:

* Адрес Веб гис - url-адрес вашей Веб ГИС (https://***.nextgis.com)
* Логин - Имя пользователя, имеющего права на запись в указанный ресурс
* Пароль - Пароль пользователя в Веб ГИС
* Идентификатор группы ресурса - Идентификатор ресурса в Веб ГИС, в котором содержится группа слоев
...
.. note::
    Год начала и год окончания - необязательные параметры. Но если они вводятся, то должны быть либо 3-, либо 4-х значными.  Например: %start_%finish_rus_earl_v.1.0 должно быть указано в названии слоя как: 1245_1246_rus_earl_v.1.0. Остальные поля являются **обязательными**.

На выходе:

*  CSV файл, где представлен перечень гиперссылок на объекты которые были изменены, значение которое заменили, значение на которое заменили.
