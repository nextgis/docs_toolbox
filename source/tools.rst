...
Инструмент разбивает все объекты полигонального слоя на равные по площади части.
...
Обрезает один полигональный слой другим. Слои могут содержать как один, так и несколько объектов и быть в разных системах координат.

На входе:

* Обрезаемый полигональный слой - файл в поддерживаемом GDAL формате, например, GeoPackage, GeoJSON, MapInfo TAB, ESRI Shapefile (последний - в ZIP-архиве).
* Обрезающий полигональный слой - файл в поддерживаемом GDAL формате, например, GeoPackage, GeoJSON, MapInfo TAB, ESRI Shapefile (последний - в ZIP-архиве).

На выходе:

* Векторный файл с обрезанными полигонами.
