...
------------------------------------------
Конвертирует таблицу XLSX с данными о захоронении грунтов в векторный файл

На входе:

Таблица XLSX – содержит координаты точек или полигонов захоронений (описываются набором точек). Чаще всего одному захоронению соответствует одна строка таблицы. 

* Шапка колонки, где указываются координаты, должна содержать слово "координаты" или "coordinates". Координаты всех точек, относящихся к одному объекту, помещаются в одну ячейку. Сами координаты должны быть представлены в формате: номер точки (опционально), широта, долгота. Поддерживаемые системы координат: СК-42 (Пулково-42) и WGS-84 - должны быть упомянуты в ячейке с координатами; при отсутствии указания на систему координат по умолчанию координаты считаются приведенными в WGS-84. Формат записи координат: десятичные градусы (58,124861) или градусы, минуты, секунды (28°44′32.9″).
* Если полигон захоронения является окружностью с известными центром и радиусом, то сначала должна идти информация о радиусе, затем информация о координатах центра. Поддерживаются длины в морских милях, кабельтовых (кбт), метрах (м) и километрах (км).
* При необходимости указания информации о вырезании из полигона места, в котором нельзя производить захоронение, необходимо указать его точки после точек основного полигона с ключевыми словами «не производить захоронение», «исключить» или «исключенного из участка».
* При необходимости присоединения полигона к имеющемуся, необходимо указать его точки после основного полигона с ключевыми словами «используемого», «использовать», «соединить» или «объединить». 
