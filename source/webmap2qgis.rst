...
* Охват - по умолчанию берётся охват, заданный в настройках карты. При желании можно указать другой. Задается в EPSG:4326, формат: низ, лево, верх, право (юг, запад, север, восток). Значения через запятую, разделитель целой и дробной части - точка. Например: ``54.5, 102.5, 59, 116``.
